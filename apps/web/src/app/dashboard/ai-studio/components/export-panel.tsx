...
import {
  ...
  Sliders, Type, Droplets, Smartphone, Square, Monitor,
  ChevronDown, ChevronUp, Scissors, Camera,
} from "lucide-react";
...
export function ExportPanel({ clip, videoFile, onClose }: ExportPanelProps) {
  const { exportClip, exportThumbnail, status, error, progress } = useClipExporter();
  const { detectSilence, isAnalyzing, result: silenceResult } = useSilenceDetector();

  // Format
  const [format, setFormat] = useState<"original" | "vertical" | "square">("original");
  ...
  // Caption
  const [captionStyle, setCaptionStyle] = useState<CaptionOpts["style"]>("none");
  ...
  const [open, setOpen] = useState<Record<Section, boolean>>({
    format: true, color: false, caption: false, silence: false, watermark: false,
  });
  ...
  const handleExport = () => {
    const opts: ExportOptions = {
      ...
      caption: captionStyle !== "none"
        ? { text: captionText, style: captionStyle, fontSize: captionFontSize, color: captionColor }
        : undefined,
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <div>
            ...
            <p className="text-xs text-muted-foreground truncate max-w-[260px]">{clip.title}</p>
          </div>
          <button
            type="button"
        ...
        <div className="overflow-y-auto flex-1">
          ...
          {open.format && (
            <div className="px-5 pb-4 grid grid-cols-3 gap-2">
              {[
                { id: "original", label: "Original", icon: <Monitor className="w-5 h-5" /> },
                { id: "vertical", label: "Vertical 9:16", icon: <Smartphone className="w-5 h-5" /> },
                { id: "square", label: "Quadrado", icon: <Square className="w-5 h-5" /> },
              ].map(f => (
                <button
                  key={f.id}
                  type="button"
            ...
            <div className="px-5 pb-4 space-y-4">
              ...
              <div className="space-y-3">
                <SliderRow
                  ...
                  onChange={v => { setColorPreset(-1); setColorGrading(c => ({ ...c, brightness: v })); }}
                />
                <SliderRow
                  label="Contraste"
                  value={colorGrading.contrast}
                  min={0.5} max={2.5} step={0.05}
                  onChange={v => { setColorPreset(-1); setColorGrading(c => ({ ...c, contrast: v })); }}
                />
                <SliderRow
                  label="Saturação"
                  value={colorGrading.saturation}
                  min={0} max={3} step={0.05}
                  onChange={v => { setColorPreset(-1); setColorGrading(c => ({ ...c, saturation: v })); }}
                />
              </div>
            </div>
          )}
          ...
          {open.caption && (
            <div className="px-5 pb-4 space-y-3">
              ...
              {captionStyle !== "none" && (
                <>
                  <div>
                    <Label className="text-xs text-muted-foreground mb-1 block">Texto da legenda</Label>
                    <Textarea
                      value={captionText}
                  ...
                  />
                  <div>
                    <Label className="text-xs text-muted-foreground mb-1.5 block">Cor do texto</Label>
                    <div className="flex gap-2">
                      {["white", "yellow", "#00ff00", "red", "#1DA1F2"].map(c => (
                        <button
                          key={c}
                          type="button"
                          ...
                          className={cn(
                            ...
                            captionColor === c ? "border-primary scale-110" : "border-transparent"
                          )}
                          style={{ background: c }}
                        />
                      ))}
          ...
          {open.silence && (
            <div className="px-5 pb-4 space-y-3">
              ...
              >
                {isAnalyzing
                  ? <><Loader2 className="w-4 h-4 animate-spin" />Analisando áudio...</>
                  : <><Sliders className="w-4 h-4" />Detectar silêncios</>
                }
              </Button>
              ...
              {silenceResult && silenceRun && (
                <div className="bg-muted/50 rounded-xl p-3 space-y-1.5">
                  <p className="text-xs font-semibold text-foreground">
                    {silenceResult.silentSegments.length} trecho(s) silencioso(s) encontrado(s)
                  </p>
                  ...
                  <div className="max-h-32 overflow-y-auto space-y-1 mt-2">
                    {silenceResult.silentSegments.map((seg) => (
                      <div key={`${seg.start}-${seg.end}`} className="flex items-center justify-between text-xs bg-background rounded-lg px-3 py-1.5">
                        <span className="text-muted-foreground">{formatTime(seg.start)} → {formatTime(seg.end)}</span>
                        <span className="font-medium text-red-500">−{seg.duration.toFixed(1)}s</span>
                      </div>
                    ))}
                  </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5">
                  ...
                  {status === "loading-ffmpeg" ? "Carregando processador de vídeo..." : "Processando clip..."}
                </span>
                <span>{progress}%</span>
              </div>
          ...
          <div className="flex gap-2">
            ...
            >
              {isProcessing
                ...
                : <><Download className="w-4 h-4" />Exportar clip</>
              }
            </Button>
          </div>
        </div>

...
import { History, Video, ArrowRight, Loader2, Calendar, FileVideo } from "lucide-react";
...
interface HistoryDialogProps {
  onSelect: (result: any, videoName: string) => void;
}
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Recupere clips e transcrições de vídeos que você já analisou anteriormente.
          </DialogDescription>
        </DialogHeader>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3 text-center text-muted-foreground">
              ...
              <div>
                ...
                <p className="text-xs">Vídeos analisados no AI Studio aparecerão aqui.</p>
              </div>
            </div>
          ) : (
            history.map(item => (
              ...
              >
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-sm truncate">{item.videoName}</h4>

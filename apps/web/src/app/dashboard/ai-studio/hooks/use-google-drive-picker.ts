...
type GooglePickerNamespace = {
	...
	picker: {
		...
		View: new (id: string) => unknown;
		PickerBuilder: new () => {
			addView: (
				view: unknown,
			) => GooglePickerNamespace["picker"]["PickerBuilder"];
			setOAuthToken: (
				token: string,
			) => GooglePickerNamespace["picker"]["PickerBuilder"];
			setDeveloperKey: (
			...
			setCallback: (
				cb: (data: GooglePickerCallbackData) => void,
			) => GooglePickerNamespace["picker"]["PickerBuilder"];
...
export function useGoogleDrivePicker() {
	...
	const pickFile = useCallback(async (): Promise<File | null> => {
		if (!CLIENT_ID || CLIENT_ID.includes("SEU_CLIENT_ID")) {
			setError(
				"Google Client ID não configurado. Adicione NEXT_PUBLIC_GOOGLE_CLIENT_ID no .env",
			);
			return null;
		}
		if (!API_KEY || API_KEY.includes("SUA_API_KEY")) {
			setError(
				"Google API Key não configurada. Adicione NEXT_PUBLIC_GOOGLE_API_KEY no .env",
			);
			return null;
		}
		...
		try {
			const [accessToken] = await Promise.all([getAccessToken(), loadGapi()]);

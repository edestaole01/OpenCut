...
export function useClipExporter() {
  ...
  const exportClip = useCallback(async (opts: ExportOptions) => {
    ...
    try {
      ...
      if (opts.colorGrading) {
        ...
        const hasEffect = brightness !== 0 || contrast !== 1 || saturation !== 1;
        if (hasEffect) {
          const eqFilter = `eq=brightness=${brightness.toFixed(2)}:contrast=${contrast.toFixed(2)}:saturation=${saturation.toFixed(2)}`;
          // If we already have a chain (overlay), we append it to the overlay result
          vfilters.push(eqFilter);
        }
      }

      // Caption burn-in
      if (opts.caption && opts.caption.style !== "none" && opts.caption.text) {
      ...
      if (opts.speechSegments && opts.speechSegments.length > 0) {
        ...
        if (clipSpeech.length > 0) {
          const selectParts = clipSpeech.map(s => `between(t,${s.start.toFixed(3)},${s.end.toFixed(3)})`).join("+");
          const vselect = `select='${selectParts}',setpts=N/FRAME_RATE/TB`;
          const aselect = `aselect='${selectParts}',asetpts=N/SR/TB`;
          vfilters.push(vselect);
          afilter = aselect;
        }
      }
      ...
      if (vfilters.length > 0) {
        const hasComplex = vfilters.some(f => f.includes("[") || f.includes(";"));
        if (hasComplex) {
          args.push("-filter_complex", vfilters.join(","));
        } else {
      ...
      const clipTitle = opts.clip.title.replace(/[^a-zA-Z0-9]/g, "_").slice(0, 30);
  ...
  const exportThumbnail = useCallback(async (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      video.addEventListener("loadedmetadata", () => { video.currentTime = atSecond; });

...
const PLATFORMS = [
  ...
  { id: "tiktok", label: "TikTok", icon: () => <span className="text-sm font-bold">TK</span>, color: "text-foreground" },
...
export default function CaptionsPage() {
    ...
    fetch("/api/captions")
      ...
      .then(data => {
        if (Array.isArray(data)) {
          setHistory(data.sort((a: GeneratedCaption, b: GeneratedCaption) =>
  ...
  const handleGenerate = async () => {
    if (!clipTitle.trim()) { toast.error("Título do clip é obrigatório"); return; }
    setLoading(true);
    setGeneratedText("");
    try {
      ...
      toast.error(e.message || "Erro ao gerar caption");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">IA gera captions otimizadas para cada plataforma</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-5">
                {PLATFORMS.map(p => {
                  ...
                  return (
                    ...
                    >
                      <span className={p.color}><Icon className="w-5 h-5" /></span>
                      {p.label}
                    </button>
                  );
                })}
          ...
          >
            {loading
              ? <><Loader2 className="w-4 h-4 animate-spin" />Gerando caption...</>
              : <><Sparkles className="w-4 h-4" />Gerar Caption para {platformLabel(platform)}</>
            }
          </Button>
        </div>
        ...
        <div className="space-y-6">
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="bg-muted/30 rounded-lg p-4">
                  <pre className="whitespace-pre-wrap text-sm font-sans leading-relaxed">{generatedText}</pre>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center h-40 gap-2 text-muted-foreground">
                  <Sparkles className="w-8 h-8 opacity-30" />
                  <p className="text-sm">Preencha as informações e clique em Gerar</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Dicas para {platformLabel(platform)}</CardTitle>
            </CardHeader>
            <CardContent>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-medium text-sm truncate">{item.clipTitle}</span>
...
function PlatformTips({ platform }: { platform: string }) {
  ...
  return (
    <ul className="space-y-2">
      {list.map((tip) => (
        <li key={`${platform}-${tip}`} className="flex items-start gap-2 text-sm text-muted-foreground">

...
import { generatedCaptions, companyProfiles, aiUsageLog } from "@/lib/db/schema";
import { eq } from "drizzle-orm";
import { nanoid } from "nanoid";

const GEMINI_API_BASE = "https://generativelanguage.googleapis.com";
const MODELS = ["gemini-2.0-flash", "gemini-1.5-flash", "gemini-1.5-pro", "gemini-flash-latest"];
...
export async function POST(req: NextRequest) {
  ...
  const { platform, clipTitle, transcript, caption: clipCaption, score, tone, companyName, targetAudience, industry } = body;

  if (!platform || !clipTitle) {
    return NextResponse.json({ error: "platform e clipTitle são obrigatórios" }, { status: 400 });
  }
  ...
  if (!companyName) {
    const [profile] = await db.select().from(companyProfiles).where(eq(companyProfiles.userId, session.user.id));
    if (profile) {
...
Gere a caption otimizada para ${platform}. Responda SOMENTE com a caption, sem explicações adicionais.`;
  ...
  if (!generated) {
    return NextResponse.json({ error: "Falha ao gerar caption com IA" }, { status: 500 });
  }

  // Salva no banco
  const [saved] = await db.insert(generatedCaptions).values({
    id: nanoid(),
...
export async function GET(req: NextRequest) {
  ...
  const list = await db.select().from(generatedCaptions)

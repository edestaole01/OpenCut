...
function extractConfirmToken(
	html: string,
): { confirm: string; uuid: string } | null {
	// Tenta extrair do link de download direto
	const linkMatch =
		html.match(/href="(\/uc\?[^"]*confirm=([^"&]+)[^"]*)"/) ||
		html.match(
			/href="(https:\/\/drive\.google\.com\/uc\?[^"]*confirm=([^"&]+)[^"]*)"/,
		);
	if (linkMatch) {
...
export async function GET(request: NextRequest) {
	...
	if (!rawInput) {
		return NextResponse.json(
			{ error: "fileId ou url obrigatório" },
			{ status: 400 },
		);
	}
	...
	if (!fileId) {
		return NextResponse.json(
			{
				error:
					"Não foi possível extrair o ID do arquivo. Use o link de compartilhamento do Google Drive.",
			},
			{ status: 400 },
		);
	}
	...
	try {
		...
		return NextResponse.json(
			{ error: "Erro de rede ao acessar Google Drive." },
			{ status: 500 },
		);
	}
	...
	if (ct2.includes("text/html")) {
		...
		if (html.includes("accounts.google.com") || html.includes("ServiceLogin")) {
			return NextResponse.json(
				{
					error:
						"Arquivo privado. No Google Drive, clique em 'Compartilhar' → 'Qualquer pessoa com o link' e tente novamente.",
				},
				{ status: 403 },
			);
		}
		...
		return NextResponse.json(
			{
				error:
					"Não foi possível baixar automaticamente. Tente baixar o vídeo do Drive e fazer upload manual.",
			},
			{ status: 422 },
		);
	}
